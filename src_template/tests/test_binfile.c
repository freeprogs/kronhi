...
int main(void)
{
    ...
    if (CU_add_test(suite, "can get size",
     ...
     || CU_add_test(suite, "can set and get name field",
     ...
     || CU_add_test(suite, "can set and get ctrlsum field",
...
void test_can_set_and_get_type_sign_field(void)
{
    ...
    binfile_type_set(&file, 'f');
    out = 0;
    binfile_type_get(&file, &out);
    CU_ASSERT_EQUAL(out, 'f');
...
void test_can_set_and_get_name_field(void)
{
    ...
    binfile_name_set(&file, "abc");
    *out = '\0';
    binfile_name_get(&file, out);
...
void test_can_set_and_get_ctrlsum_field(void)
{
    ...
    binfile_ctrlsum_set(&file, 0x11223344);
    out = 0;
    binfile_ctrlsum_get(&file, &out);
    CU_ASSERT_EQUAL(out, 0x11223344);
