...
int main(void)
{
    ...
    suite = CU_add_suite("chain create dir", NULL, NULL);
    if (suite == NULL) {
        CU_cleanup_registry();
        return CU_get_error();
    }

    if (CU_add_test(suite, "raise on very big file offset",
                    test_raise_on_very_big_file_offset) == NULL ||
        CU_add_test(suite, "raise on very big directory size",
                    test_raise_on_very_big_directory_size) == NULL ||
        CU_add_test(suite, "can write directory to the end of file",
