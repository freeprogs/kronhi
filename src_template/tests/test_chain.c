...
int main(void)
{
    ...
    suite4 = CU_add_suite("chain append file xor", NULL, NULL);
    if (suite4 == NULL) {
        CU_cleanup_registry();
        return CU_get_error();
    }
...
void test_raise_on_append_file_to_broken_file(void)
{
    ...
    /* assert: first file has been prepared and written on
       destination */
    ...
    /* assert: source file has not been written to directory on
       destination */
